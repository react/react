...
// The current Fiber for a key from getNodeKey.
function getFiberForNodeKey(key: mixed): Fiber {
  // Either a reconciler FiberInstance, which has `current`, or a Fiber.
  const keyAsAny: any = key;
  return keyAsAny.current !== undefined ? keyAsAny.current : keyAsAny;
}
  ...
  function updateComponentFilters(componentFilters: Array<ComponentFilter>) {
    ...
    const previousForcedFallbacks =
      forceFallbackForFibers.size > 0
        ? new Set(forceFallbackForFibers.values())
        : null;
    ...
    if (typeof scheduleUpdate === 'function') {
      ...
      if (
        ...
        // eslint-disable-next-line no-for-of-loops/no-for-of-loops
        for (const [key, shouldError] of previousForcedErrors) {
          forceErrorForFibers.set(key, false);
          if (shouldError) {
            const fiber = getFiberForNodeKey(key);
            if (typeof scheduleRetry === 'function') {
              scheduleRetry(fiber);
            } else {
              scheduleUpdate(fiber);
