...
describe('ReactFlightTurbopackLedgers', () => {
  ...
  it('creates a capture before rendering and serializes its total before its data', async () => {
    const Tags = ReactServer.createSetLedger();

    async function Page() {
      ReactServer.addToLedger(Tags, 'before');
      await Promise.resolve();
      ReactServer.addToLedger(Tags, 'after');
      return 'page';
    }
    ...
    expect(Array.from(await result.tags).sort()).toEqual(['after', 'before']);
  });
  ...
  // @gate enableFlightLedgers
  it('renders a capture created in an earlier request without carrying over its total', async () => {
    const Tags = ReactServer.createSetLedger();
    let captured;
    ...
    function FirstApp() {
      captured = ReactServer.captureLedgers(<Page />, [Tags]);
      // The handle is serialized, but the data is never rendered here.
      return {tags: captured.ledgers[0]};
    }

    const first = await render(<FirstApp />);
    expect(Array.from(await first.tags)).toEqual([]);
    ...
    expect(Array.from(await second.tags)).toEqual(['page']);
    expect(Array.from(await first.tags)).toEqual([]);
  });

  // @gate enableFlightLedgers
  it('keeps concurrent renders of the same capture independent', async () => {
    const Tags = ReactServer.createSetLedger();
    let resolveFirst;
    const firstEnd = new Promise(resolve => {
      resolveFirst = resolve;
    });
    let resolveSecond;
    const secondEnd = new Promise(resolve => {
      resolveSecond = resolve;
    });
    let renders = 0;

    async function Page() {
      const name = ++renders === 1 ? 'first' : 'second';
      ReactServer.addToLedger(Tags, name + '-before');
      await (name === 'first' ? firstEnd : secondEnd);
      ReactServer.addToLedger(Tags, name + '-after');
      return name;
    }

    const captured = ReactServer.captureLedgers(<Page />, [Tags]);
    const model = {tags: captured.ledgers[0], page: captured.data};
    ...
    const first = await ReactServerDOMClient.createFromReadableStream(
      firstStream,
      {
        serverConsumerManifest,
      },
    );
    const second = await ReactServerDOMClient.createFromReadableStream(
      secondStream,
      {
        serverConsumerManifest,
      },
    ...
    await serverAct(() => resolveSecond());
    expect(Array.from(await second.tags).sort()).toEqual([
      'second-after',
      'second-before',
    ]);
    expect(firstSettled).toBe(false);

    await serverAct(() => resolveFirst());
    expect(Array.from(await first.tags).sort()).toEqual([
      'first-after',
      'first-before',
    ]);
  });
  ...
  it('passes a ledger the nested capture does not declare through to the enclosing one', async () => {
    ...
    function Layout() {
      ...
      // The inner handle is not serialized, but it must still shadow Tags.
      return {segment: captured.data};
    }
    ...
    const result = await render(<App />);
    expect(result.layout.segment).toBe('segment');
