...
describe('ReactFlightTurbopackLedgersHosts (browser)', () => {
  ...
  // Creation does not need a renderer, but consuming the capture needs a
  // host that can track writes across async work.
  // @gate enableFlightLedgers
  it('rejects a capture created outside rendering on an unsupported host', async () => {
    const Tags = ReactServer.createSetLedger();
    ...
    expect(errors).toEqual([
      'Cannot render captured ledgers because this Server Components ' +
        'renderer has no async context. Use a Node or ' +
