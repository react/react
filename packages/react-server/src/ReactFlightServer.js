...
// Reference paths still this short are built eagerly: extending a short
// string is as cheap as recording a lazy entry, and dedupe hits tend to
// land on shallow objects (normalized data referenced from several
// places), where materializing on every hit would cost more than eager
// construction ever did.
const EAGER_REFERENCE_LIMIT = 24;

function registerLazyReference(
  request: Request,
  parentEntry: string | LazyReference,
  key: string,
): string | LazyReference {
  if (
    typeof parentEntry === 'string' &&
    parentEntry.length < EAGER_REFERENCE_LIMIT
  ) {
...
function renderModelDestructive(
  ...
  if (typeof value === 'object') {
    switch ((value as any).$$typeof) {
      case REACT_ELEMENT_TYPE: {
        let elementReference: null | string | LazyReference = null;
